...
class SimLang
  def initialize
    @langParser = Parser.new("SimLang") do
      ...
      rule :assignment do 
        match('set', '#', :variable, 'to', :expr) { |_, _, variable, _, value| Assignment.new(variable, value) }
      end
      ...
      rule :inner_statements do
        match(:inner_statements, :statement) { |inner, stmt| inner.add_stmt(stmt) }
        match(:statement) { |inner| Statements.new([inner]) }
      end
      ...
      rule :logical_expression do
        match(:logical_term)
        match(:logical_expression, 'or', :logical_term) { |expr1, _, expr2| LogicalExpression.new(expr1, :or, expr2) }
      end

      rule :logical_term do
        match(:logical_factor)
        match(:logical_term, 'and', :logical_factor) { |term, _, factor| LogicalExpression.new(term, :and, factor) }
      end

      rule :logical_factor do
        match('not', :logical_factor) { |_, factor| LogicalExpression.new(:not, factor) }
      ...
      rule :function do
        match('function', :variable, '(', :parameters, ')', :block) { |_, name, _, params, _, block| FunctionDeclaration.new(name, params, block) }
      end
      
      rule :parameters do
        match(:parameters, ',', :variable) { |params, _, param| params << param }
        match(:variable) { |param| [param] }
        match() { [] }
      end

      rule :function_call do
        match(:variable, '(', :variable, ')') { |func_name, _, args, _| FunctionCall.new(func_name, args) }
      end
