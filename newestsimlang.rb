  ...
  def initialize
    @langParser = Parser.new("SimLang") do
      ...
      rule :if_statement do 
        match('if', '(', :expr, ')', 'then', :block) { |_, _, expr, _, _, block| IfStatement.new(expr, block)}
        match('if', '(', :expr, ')', 'then', :block, 'else', :block) { |_, _, expr, _, _, block, _ , block2| IfStatement.new(expr, block, block2)}
      end
