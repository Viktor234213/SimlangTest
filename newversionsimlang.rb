...
class SimLang
  def initialize
    @langParser = Parser.new("SimLang") do
      ...
      rule :expr do
        match(:primary, :comparison_operator, :expr)
        match(:primary)
      end
      ...
      rule :logical_expression do
        match(:logical_term)
        match(:logical_expression, 'or', :logical_term) { |expr1, operator , expr2| LogicalExpression.new(expr1, operator, expr2) }
      end

      rule :logical_term do
        match(:logical_factor)
        match(:logical_term, 'and', :logical_factor) { |term, operator, factor| LogicalExpression.new(term, operator, factor) }
      end

      rule :logical_factor do
        match('not', :logical_factor) { |operator, factor| LogicalExpression.new(operator, factor) }
